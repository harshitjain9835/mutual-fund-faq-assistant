...
# Perform a single check at startup to see if the database is populated.
# This avoids running .count() on every user query.
IS_DB_EMPTY = collection.count() == 0
if IS_DB_EMPTY:
    print("WARNING: ChromaDB collection 'mutual_fund_facts' is empty.")
    print("Please run 'python src/ingest.py' followed by 'python src/retrieval.py index' to populate the database.")
...
def clear_and_rebuild_index() -> None:
    """Clears the existing ChromaDB collection and rebuilds it from the latest ingestion."""
    global collection
    try:
        client.delete_collection(name="mutual_fund_facts")
    except Exception as e:
        print(f"Could not delete old collection (might not exist): {e}")
    collection = client.get_or_create_collection(name="mutual_fund_facts")
    build_index()
...
def build_index() -> None:
    ...
    if not corpus_file.exists():
        raise FileNotFoundError(
            ...
            "Please run the ingestion script first (e.g., 'python src/ingest.py')."
        )
...
def retrieve_passages(query: str, top_k: int = 3) -> List[Dict[str, Any]]:
    ...
    # Fail fast if the database was found to be empty at startup.
    # This prevents a long wait for the user if the index hasn't been built.
    if IS_DB_EMPTY:
        return [{"error": "unavailable", "message": "The vector database is empty. Please ask the administrator to build the search index."}]
